...
mDASH\cite{7393865} formulate rate adaptation problem as Markov Decision Process (MDP)\cite{P-1066} optimization. According to Zhou \etal\, the state vector defien the system state including buffer occupancy, playback rate, previous video bitrate, network bandwidth. The action is the bitrate for next segment. They calculate reward based on various parameters that affect the visual quality which includes quality, quality switch frequency and magnitude, rebuffering. The optimization problem is difficult to solve directly as there are lotof uncertainaty during the streaming session due to time-varying network condition. So, authors have propose a sub-optimal greedy solution. According the their evalution mDASH perform at per with the optimal solution.
